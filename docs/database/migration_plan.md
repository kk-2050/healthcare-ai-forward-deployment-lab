...
- The `processing_step` → `workflow_step_id` translation (§5.B) requires new mapping logic between LangGraph step names and `workflow_definition_steps.step_code`; underestimating this as a "rename" would produce an incorrect Wave 2 estimate. This is tracked as an Open Decision (§16), due before Wave 2.
...
- **No Alembic migration revision exists yet (§16).** The framework (Alembic + SQLAlchemy, per [ADR-005](../decisions/ADR-005-database-schema-migration-strategy.md)) is installed and initialized, but the first-revision strategy for the existing brownfield `workflow_runs`/`audit_events` tables has not been decided. This blocks the *first physical* Wave 1 schema change — not Wave 1 planning, which is unblocked (see §17). Beginning physical implementation before a first-revision strategy is decided risks an incorrect or destructive first migration.
...
- Alembic + SQLAlchemy, the approved schema-application mechanism ([ADR-005](../decisions/ADR-005-database-schema-migration-strategy.md)), is now installed and initialized (Task 19E); a separate, explicitly approved task will decide the first-revision strategy for the existing brownfield tables before Wave 1 physical implementation begins (see §16).
...
|---|---|---|---|
| First-revision strategy for the existing brownfield `workflow_runs`/`audit_events` tables — how the Alembic revision history should support both the existing local database and a fresh database built from zero (see Task 19E's Brownfield/Fresh-Build note) | OPEN | Wave 1 **physical implementation** (the first actual migration revision) | **Blocks the first physical schema change.** Does not block Wave 1 planning. Must not be solved by stamping or destructively resetting the existing database without explicit approval. |
...
No further decisions are required to complete Wave 0. None of the three remaining OPEN items block Wave 1 **planning**. The schema-application mechanism's *architecture decision* and the Alembic *framework installation* are both resolved; the *first-revision strategy* is the item that now gates Wave 1 **physical implementation** — see §17 for that distinction.
...
- [x] Identifier strategy is confirmed compliant in format, with the generation-point gap tracked as an Open Decision due before Wave 2, not Wave 1 (§8, §16).
...
- [x] No unresolved MUST-FIX blocker exists **for planning purposes** (§14 lists risks; the schema-application mechanism's *architecture decision* is resolved via ADR-005 and the Alembic *framework* is installed and initialized (Task 19E) — only the *first-revision strategy* for the brownfield tables remains outstanding, and that blocks physical implementation, not planning; see 17.B).
...
The schema-application mechanism **architecture decision** is resolved (ADR-005), and the Alembic **framework is installed and initialized** (Task 19E): `alembic.ini` and `migrations/` (`env.py`, `script.py.mako`, `versions/`) exist, `env.py` uses this project's existing `Base.metadata` and existing secure database configuration, and Alembic 1.20.0 is pinned in `requirements.txt`. Neither of these makes Wave 1 physical implementation ready by itself — **no migration revision exists, and the strategy for the first revision has not been decided.** The existing local database already contains the Task 18B brownfield prototype tables (`dbo.workflow_runs`, `dbo.audit_events`); a future task must explicitly decide how the Alembic revision history supports both that existing database and a fresh database built from zero, before any revision is written. No `CREATE TABLE`, `ALTER TABLE`, `alembic upgrade`, or `alembic stamp` against `healthcare_ai_fde_lab` — even for the comparatively simple Wave 1 reference-master tables — should occur before that first-revision strategy is decided and a migration-design task is completed and validated.

**Wave 1 planning: UNBLOCKED. Architecture decision for the migration mechanism: RESOLVED (ADR-005). Alembic framework installation/initialization: COMPLETED (Task 19E). First-revision strategy for the brownfield tables: NOT YET DECIDED. Therefore Wave 1 physical DDL must not start until a separate migration-design task decides that strategy and is completed and validated.**
