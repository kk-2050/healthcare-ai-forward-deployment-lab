...
| Reference-data loading implementation (how `reference_data.md`'s controlled seed values are actually loaded into each new table) | OPEN | Whichever Wave first requires seeded reference data to be queryable, not decided here | Does not block Wave 1 migration authoring itself; tracked separately per [ADR-005](../decisions/ADR-005-database-schema-migration-strategy.md)'s Reference Data Policy (schema migration and seed-data loading are separate concerns). |

No further decisions are required to complete Wave 0. None of the remaining OPEN items block Wave 1 **planning**. The schema-application mechanism's *architecture decision*, the Alembic *framework installation*, and the *first-revision/brownfield strategy* are all resolved — see §17 for what that does and does not unblock.
