...
- **Alembic + SQLAlchemy** for physical SQL Server schema migration and
  ...
  — this is a **decided architecture choice, not yet implemented**:
  Alembic is not installed, `alembic.ini`/`migrations/` do not exist,
  and no migration has been run. Do not substitute a different
  migration mechanism without explicit approval, and do not install or
  initialize Alembic outside an explicitly approved implementation
  task.
...
- Once Alembic is introduced (see Fixed Technology Decisions and
